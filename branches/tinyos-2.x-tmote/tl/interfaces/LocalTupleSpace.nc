 ...
 * *
 * *	$Id: LocalTupleSpace.nc 246 2007-12-22 21:03:32Z lmottola $
 * *
...
interface LocalTupleSpace {
  ...
  event error_t reifyCapabilityTuple(tuple* ct, TLOpId_t operationId);

  // Request to reify a capability tuple
  event tuple* reifyNeighborTuple();

  // Returning tuples
  event error_t tupleReady(TLOpId_t operationId, 
			    tuple *tuples, 
			    uint8_t number);
}
