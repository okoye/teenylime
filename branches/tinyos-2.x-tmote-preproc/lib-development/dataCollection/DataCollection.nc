...
implementation {
  ...
  components SchedulerC as SC;

  components new TimerMilliC() as TimerUR;

#ifdef CLASS_1
  components new TimerMilliC() as Rate;
#endif

  components new TimerMilliC() as TimerReliablePath;
  components new TimerMilliC() as TimeoutRouteInfo;
  ...
  TB.TimeoutRouteInfo -> TimeoutRouteInfo;
  ...
  SC.TLObjects -> TLObjectsParsed;
  SC.TokenInfo -> DF.TokenInfo;
#ifdef CLASS_1
  SC.Rate -> Rate;
#endif
  ...
  SC.PrintfControl = PrintfControl;
  SC.PrintfFlush = PrintfFlush;
