...
/*                    active forwarding */
                   ...
                   signal TokenInfo.forwardingActive();
                 }
               });
...
/*              If the out operation failed */
             type = (forwarding_msg.value3[0] << 8) + forwarding_msg.value3[1];
             if (forwarding_msg.value2 == UNRELIABLE_DELIVERY){
               traffic_class = 0;
             } else if (type == CLASS_1_END_SESSION){
               traffic_class = 1;
             } else if (type == CLASS_1_TYPE){
               traffic_class = 1;
...
/*                insert the message in the local cache */
             ...
             rem_outing = FALSE;
/*             Notify the scheduler to put a new token based on class*/
             if (ID == TL_LOCAL)
               signal TokenInfo.LocalMsgSent(traffic_class);
...
/*                look for a new message to forward */
  ...
  event void TreeConnection.parentUpdate(uint16_t parent){
    ...
    if (current_parent != parent || fw_active){
      signal CollectionDebug.parentUpdated(call TreeConnection.getGateway(),
                                           parent, 
  ...
  event void TreeConnection.congestedPath(bool root, bool failure){
    ...
    atomic{
      if (notified_active){
        notified_active = FALSE;
        signal TokenInfo.forwardingInactive();
      }
      if (fw_active){
        fw_active = FALSE;
      }
  ...
  command uint16_t CollectionInfo.gateway(){
    return call TreeConnection.getGateway();
  }
  ...
  default event void CollectionDebug.parentUpdated(uint16_t gateway,
                                                   uint16_t parent, 
                                                   uint16_t cost){}
