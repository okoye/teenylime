...
// Diode selection
#define DONE		0x00		// is the photodiode sensitive to both visible and infrared light
#define DTWO		0x04		// is the photodiode sensitive to infrared light
#define DONE_DTWO	0x08		// automatic reading of both photodiodes and subtraction of photodiode1 - photodiode2 (infrared compensation)
...
{	
 ...
 void wait() {
  ...
  norace uint16_t IRdata_read1;
  norace uint16_t IRdata_read2;
  norace uint16_t IRdata_read3;
  norace uint16_t IRdata_read4;

  norace uint8_t sensors;
  
  uint8_t sensors_started;

  task void readDone_task(){atomic{signal Read.readDone(result,data_read1,data_read2,data_read3,data_read4,IRdata_read1,IRdata_read2,IRdata_read3,IRdata_read4);}}

  command uint8_t Read.read(uint8_t set_sensors)
  {
  ...
  {	
	...
	{
		case 2:
			if( (next_operation & SENSOR1) && (sensors & IR_SENSOR1))
			{
				// Reading of D2
				if(read_word(ADDR1,0x04, & IRdata_read1))
				{
					result |= SENSOR1_ERROR;
				...
				if(next_operation & SENSOR1)
				{
						...
						if(write_byte(ADDR1,0x00,POWER_DOWN | NUMBER_OF_BIT | DTWO))
							result |= SENSOR1_ERROR;
					#endif
				}
			}
			
			if( (next_operation & SENSOR2) && (sensors & IR_SENSOR2))
			{
				// Reading of D2
				if(read_word(ADDR2,0x04, & IRdata_read2))
				{
					result |= SENSOR2_ERROR;
				...
				{
					#ifdef POWER_SAVING_MODE
						if(write_byte(ADDR2,0x00,POWER_DOWN | NUMBER_OF_BIT | DTWO))
							result |= SENSOR2_ERROR;
					#endif
				}
			}
			
			if( (next_operation & SENSOR3) && (sensors & IR_SENSOR3))
			{
				// Reading of D2
				if(read_word(ADDR3,0x04, & IRdata_read3))
				{
					result |= SENSOR3_ERROR;
				...
				{
					#ifdef POWER_SAVING_MODE
						if(write_byte(ADDR3,0x00,POWER_DOWN | NUMBER_OF_BIT | DTWO))
							result |= SENSOR3_ERROR;
					#endif
				}
			}
			
			if( (next_operation & SENSOR4) && (sensors & IR_SENSOR4))
			{
				// Reading of D2
				if(read_word(ADDR4,0x04, & IRdata_read4))
				{
					result |= SENSOR4_ERROR;
				...
				{
					#ifdef POWER_SAVING_MODE
						if(write_byte(ADDR4,0x00,POWER_DOWN | NUMBER_OF_BIT | DTWO))
							result |= SENSOR4_ERROR;
					#endif
				}
			}
		...
		case 1:	
			if( (next_operation & SENSOR1) && (sensors & SENSOR1))
			{
				...
				{				
					...
					if(sensors & IR_SENSOR1)
					{
						IRdata_read1 = 0;
						
						// put the sensor down before change the settings
						if(write_byte(ADDR1,0x00,POWER_DOWN | NUMBER_OF_BIT | DONE_DTWO))
						{
							result |= SENSOR1_ERROR;
							next_operation &= SENSOR1_BLOCK_NEXT_OPERATION;
						}
						// select D2
						if( (next_operation & SENSOR1) && ( write_byte(ADDR1,0x00,NUMBER_OF_BIT | DTWO) ) )
						{
							result |= SENSOR1_ERROR;
							next_operation &= SENSOR1_BLOCK_NEXT_OPERATION;
						}
			...
			{	
				...
				if(next_operation & SENSOR2)
				{
					...
					if(sensors & IR_SENSOR2)
					{
						IRdata_read2 = 0;
						
						// put the sensor down before change the settings
						if(write_byte(ADDR2,0x00,POWER_DOWN | NUMBER_OF_BIT | DONE_DTWO))
						{
							result |= SENSOR2_ERROR;
							next_operation &= SENSOR2_BLOCK_NEXT_OPERATION;
						}
						// select D2
						if( (next_operation & SENSOR2) && ( write_byte(ADDR2,0x00,NUMBER_OF_BIT | DTWO) ) )
						{
							result |= SENSOR2_ERROR;
							next_operation &= SENSOR2_BLOCK_NEXT_OPERATION;
						}
			...
			if( (next_operation & SENSOR3) && (sensors & SENSOR3))
			{
				...
				if(next_operation & SENSOR3)
				{
					...
					if(sensors & IR_SENSOR3)
					{
						IRdata_read3 = 0;
						
						// put the sensor down before change the settings
						if(write_byte(ADDR3,0x00,POWER_DOWN | NUMBER_OF_BIT | DONE_DTWO))
						{
							result |= SENSOR3_ERROR;
							next_operation &= SENSOR3_BLOCK_NEXT_OPERATION;
						}
						// select D2
						if( (next_operation & SENSOR3) && ( write_byte(ADDR3,0x00,NUMBER_OF_BIT | DTWO) ) )
						{
							result |= SENSOR3_ERROR;
							next_operation &= SENSOR3_BLOCK_NEXT_OPERATION;
						}
			...
			if( (next_operation & SENSOR4) && (sensors & SENSOR4))
			{
				...
				if(next_operation & SENSOR4)
				{
					...
					if(sensors & IR_SENSOR4)
					{
						IRdata_read4 = 0;
						
						// put the sensor down before change the settings
						if(write_byte(ADDR4,0x00,POWER_DOWN | NUMBER_OF_BIT | DONE_DTWO))
						{
							result |= SENSOR4_ERROR;
							next_operation &= SENSOR4_BLOCK_NEXT_OPERATION;
						}
						// select D2
						if( (next_operation & SENSOR4) && ( write_byte(ADDR4,0x00,NUMBER_OF_BIT | DTWO) ) )
						{
							result |= SENSOR4_ERROR;
							next_operation &= SENSOR4_BLOCK_NEXT_OPERATION;
						}
  ...
  command uint8_t StdControl.start(uint8_t set_sensors)
  {
