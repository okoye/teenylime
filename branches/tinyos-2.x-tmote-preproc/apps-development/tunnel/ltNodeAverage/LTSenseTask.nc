 ...
 * * DATE
 * *    $LastChangedDate: 2009-07-22 11:49:26 +0200 (Wed, 22 Jul 2009) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: mceriotti $
 * *
 * *	$Id: LTSenseTask.nc 887 2009-07-22 09:49:26Z mceriotti $
 * *
...
implementation {
  ...
  uint16_t avgSamples[LIGHT_SENSORS];
  ...
  nx_struct opaqueTupleLT {
    ...
    nx_uint16_t average_light_value_S1;
    nx_uint16_t average_light_value_S2;
    nx_uint16_t average_light_value_S3;
    nx_uint16_t average_light_value_S4;
  };
  ...
  uint8_t total_reading_failures = 0;
  uint8_t total_unreliable_sensors = 0;

  bool tkReactActive, alarmToSend;
  error_t lightSensorState;
  uint8_t missingRead;
  ...
  void processData() {
    ...
    for (i = 0; i < lightSampleIx; i++) {
      sumLight += lightSamples[0][i];
    }
    if (lightSampleIx > 0)
      ot->average_light_value_S1 = (uint16_t) (sumLight / 
                                               (uint32_t) lightSampleIx);
    else
      ot->average_light_value_S1 = 0;
    
    sumLight = 0;
    for (i = 0; i < lightSampleIx; i++) {
      sumLight += lightSamples[1][i];
    }
    if (lightSampleIx > 0)
      ot->average_light_value_S2 = (uint16_t) (sumLight / 
                                               (uint32_t) lightSampleIx);
    else
      ot->average_light_value_S2 = 0;
    
    sumLight = 0;
    for (i = 0; i < lightSampleIx; i++) {
      sumLight += lightSamples[2][i];
    }
    if (lightSampleIx > 0)
      ot->average_light_value_S3 = (uint16_t) (sumLight / 
                                               (uint32_t) lightSampleIx);
    else
      ot->average_light_value_S3 = 0;
    
    sumLight = 0;
    for (i = 0; i < lightSampleIx; i++) {
      sumLight += lightSamples[3][i];
    }
    if (lightSampleIx > 0)
      ot->average_light_value_S4 = (uint16_t) (sumLight / 
                                               (uint32_t) lightSampleIx);
    else
      ot->average_light_value_S4 = 0;
