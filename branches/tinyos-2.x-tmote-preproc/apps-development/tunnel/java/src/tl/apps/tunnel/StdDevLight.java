 ...
 * *
 * *	$Id: StdDevLight.java 895 2009-09-10 09:13:45Z mceriotti $
 * *
...
public class StdDevLight implements _CollectionFeatureListener {
	...
	public StdDevLight(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public StdDevLight(String dir, boolean gui) {
		feature = new TunnelFeature(TunnelFeature.STD_DEV_LIGHT,
	...
	public void receivedSample(SourceId sourceId, Sample sample) {
		...
		if (console) {
			...
			try {
				...
				writer.write("\tSTDDEVLIGHT: " + sampleValue.get(0));
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "Light Sensor 2",
					"Light Sensor 2", "Raw Value");
		return panel;
	}
