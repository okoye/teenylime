 ...
 * *    TeenyLIME
 * * VERSION
 * *    $LastChangedRevision: 936 $
 * * DATE
 * *    $LastChangedDate: 2009-11-10 16:33:28 -0600 (Tue, 10 Nov 2009) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: mceriotti $
 * *
 * *	$Id: SourceTunnel.java 936 2009-11-10 22:33:28Z mceriotti $
 * *
...
public class SourceTunnel extends Source {
	...
	private _CollectionFeature parentF;
	private _CollectionFeature parentQualityF;

	public SourceTunnel(SourceId identifier, boolean sink,
			_CollectionFeature batteryF, _CollectionFeature treeInfo,
			_CollectionFeature parentF, _CollectionFeature parentQualityF) {
		super(identifier, sink, Properties.CACHE_SIZE);
		this.batteryF = batteryF;
		this.treeInfo = treeInfo;
		this.parentF = parentF;
		this.parentQualityF = parentQualityF;
	...
	public void updateCollectedSamples(
		...
		while (keys.hasMoreElements()) {
			...
			} else if (key.equals(parentF)) {
				Vector<Integer> value = (Vector<Integer>) samples.get(key)
						.getValue();
				this.parent = new SourceId(value.get(0));
			} else if (key.equals(parentQualityF)) {
				Vector<Integer> value = (Vector<Integer>) samples.get(key)
						.getValue();
				this.pathCost = value.get(0);
			} else if (!key.id().equals(TunnelFeature.NULL)
					&& !key.id().equals(TunnelFeature.TEMPERATURE)
					&& !key.id().equals(TunnelFeature.IR)) {
				replaceLast = true;
			}
		}
		if (replaceLast)
