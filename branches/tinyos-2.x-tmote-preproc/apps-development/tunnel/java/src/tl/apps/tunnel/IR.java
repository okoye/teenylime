 ...
 * *
 * *	$Id: IR.java 972 2009-12-03 06:40:36Z mceriotti $
 * *
...
public class IR implements _CollectionFeatureListener {
	...
	public IR(boolean gui) {
		this.gui = gui;
		this.console = !gui;
	}

	public IR(String dir, boolean gui) {
		feature = new TunnelFeature(TunnelFeature.IR, "IR");
		this.fileName = "";
		if (dir.length() > 0) {
	...
	public void receivedSample(SourceId sourceId, Sample sample) {
		...
		if (gui && panel != null) {
			for (int i = 0; i < sampleValue.size(); i += 2) {
				panel.addPoint(sourceId, sample.getSamplingPeriod()
						* (sampleValue.size()/2) + i/2, sampleValue.get(i));
			}
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new NumXYChart(scenario, "IR", "IR", "Raw Value");
		return panel;
	}
