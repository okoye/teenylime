 ...
 * *
 * *	$Id: ConsoleScenarioTunnel.java 1017 2010-01-11 08:32:29Z mceriotti $
 * *
...
import tl.apps.tunnel.TupleReaderRoutingInfo;
...
public class ConsoleScenarioTunnel implements _CollectionScenario,
	...
	private TupleReaderIR irTupleReader;
	...
	public ConsoleScenarioTunnel() {
		...
		this.ir = new IR(Properties.LOG_DIR_NAME, false);
	...
	public _CollectionTupleReader getTupleReader(Tuple tuple) {
		...
		if (pattern.matches(tuple)) {
			...
			case 0:
				...
				else if (data[1] == Constants.ROUTING_INFO_TYPE)
					return this.routingInfoTupleReader;
				break;
	...
	public Vector<_ConsoleDisseminator> getDisseminators() {
		...
		taskers.add(new TaskerClassic());
		taskers.add(new TaskerRandomErrorAllNet());
		taskers.add(new TaskerRandomError18());
		taskers.add(new TaskerLeandroError());
		taskers.add(new TaskerFiremen());
		return taskers;
	}
