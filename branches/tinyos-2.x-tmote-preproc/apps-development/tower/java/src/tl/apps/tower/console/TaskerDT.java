 ...
 * *
 * *	$Id: TaskerDT.java 980 2009-12-03 07:11:16Z mceriotti $
 * *
...
public class TaskerDT implements _ConsoleDisseminator {
	...
	public Vector<Tuple> getTuples() throws IllegalArgumentException {
		...
		System.out.println("DEFORMATION/TEMPERATURE TASK DEFINITION");
		...
		System.out.println("Insert the reporting period (RP) in minutes");
		try {
		...
		content.add(new Field().actualField(new Uint8(Constants.DT_TASK)));
		content.add(new Field().actualField(new Uint16(p)));
		content.add(new Field().actualField(new Uint16(f)));
		...
		envelope.add(new Field().actualField(new Uint16(Constants.DT_TASK)));
