 ...
 * *
 * *	$Id: TaskerVibration.java 980 2009-12-03 07:11:16Z mceriotti $
 * *
...
public class TaskerVibration implements _ConsoleDisseminator {

	private BufferedReader reader;

	public TaskerVibration() {
	...
	public Vector<Tuple> getTuples() throws IllegalArgumentException {
		...
		System.out.println("VIBRATION TASK DEFINITION\n"
				+ "(synchronization is forced before the task)");
		System.out.println("Insert the sampling rate (R) in Hz");
		try {
			r = Integer.parseInt(reader.readLine());
		} catch (Exception e) {
			throw new IllegalArgumentException(e);
		}
		System.out.println("Insert the sampling session length (S) in seconds");
		try {
		...
		System.out
				.println("Insert the total number (N) of sampling sessions (-1 for an infinite task)");
		try {
		...
		content.add(new Field()
				.actualField(new Uint8(Constants.VIBRATION_TASK)));
