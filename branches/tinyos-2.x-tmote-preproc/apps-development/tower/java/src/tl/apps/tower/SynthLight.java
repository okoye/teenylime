 ...
 * *
 * *	$Id: SynthLight.java 978 2009-12-03 07:01:27Z mceriotti $
 * *
...
public class SynthLight implements _CollectionFeatureListener {
	...
	public SynthLight(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public SynthLight(String dir, boolean gui) {
		feature = new FeatureTower(FeatureTower.SYNTH_LIGHT, "SynthLight");
		if (dir.length() > 0) {
	...
	public void receivedSample(SourceId sourceId, Sample sample) {
		...
		if (console) {
			...
			try {
				...
				writer.write("\tSYNTHLIGHT: " + sampleValue.get(0));
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "SynthLight", "SynthLight",
					"Lux");
		return panel;
	}
	...
	public static Vector<Double> convert(int rawValue) {
		...
		double synthLight = (((double) rawValue / 4096 * 1.5) / 100) * 0.769 * 1e6;
		ret.add(new Double(synthLight));
