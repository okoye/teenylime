 ...
 * *
 * *	$Id: Presence.java 883 2009-07-14 12:51:17Z mceriotti $
 * *
...
public class Presence implements _CollectionFeatureListener {
	...
	public Presence(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public Presence(String dir, boolean gui) {
		feature = new FeatureOffice(FeatureOffice.PRESENCE, "Presence");
		this.fileName = "";
		if (dir.length() > 0) {
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "Presence", "Presence", "Raw Value");
		return panel;
	}
