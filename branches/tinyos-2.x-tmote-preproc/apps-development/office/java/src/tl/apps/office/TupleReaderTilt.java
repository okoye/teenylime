 ...
 * *
 * *	$Id: TupleReaderTilt.java 883 2009-07-14 12:51:17Z mceriotti $
 * *
...
public class TupleReaderTilt implements _CollectionTupleReader {
	...
	public Hashtable<_CollectionFeature, Sample> read(Tuple tuple) {
		...
		for (int i = 0; i < size; i++) {
			...
			samples.add(Tilt.convert(sample));
			i++;
			sample = (data[9 + i * 2] << 8) + data[10 + i * 2];
			samples.add(Tilt.convert(sample));
			samples.add(new Date(time_ref));
			time_ref += time_distance;
		}
		ret.put(Tilt.getFeature(), new Sample(samples, period, new Date(System
