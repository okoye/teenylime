 ...
 * *
 * *	$Id: TupleReaderHumidity.java 883 2009-07-14 12:51:17Z mceriotti $
 * *
...
public class TupleReaderHumidity implements _CollectionTupleReader {
	...
	public Hashtable<_CollectionFeature, Sample> read(Tuple tuple) {
		...
		for (int i = 0; i < size; i++) {
			...
			samples.add(Humidity.convert(sample));
			samples.add(new Date(time_ref));
			time_ref += time_distance;
		}
		ret.put(Humidity.getFeature(), new Sample(samples, period, new Date(
