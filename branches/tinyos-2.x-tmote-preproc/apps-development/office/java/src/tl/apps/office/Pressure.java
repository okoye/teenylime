 ...
 * *
 * *	$Id: Pressure.java 883 2009-07-14 12:51:17Z mceriotti $
 * *
...
public class Pressure implements _CollectionFeatureListener {
	...
	public Pressure(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public Pressure(String dir, boolean gui) {
		feature = new FeatureOffice(FeatureOffice.PRESSURE, "Pressure");
		this.fileName = "";
		if (dir.length() > 0) {
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "Pressure", "Pressure", "Kg");
		return panel;
	}

	public static Vector<Double> convert(int rawValue) {
		Vector<Double> ret = new Vector<Double>();
		double pressure = (double) rawValue / 4096 * 2.5 * 10.56;
		ret.add(new Double(pressure));
