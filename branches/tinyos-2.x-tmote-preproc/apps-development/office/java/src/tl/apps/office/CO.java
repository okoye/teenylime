 ...
 * *
 * *	$Id: CO.java 883 2009-07-14 12:51:17Z mceriotti $
 * *
...
public class CO implements _CollectionFeatureListener {
	...
	public CO(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public CO(String dir, boolean gui) {
		feature = new FeatureOffice(FeatureOffice.CO, "CO");
		this.fileName = "";
		if (dir.length() > 0) {
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "CO", "CO", "Raw Value");
		return panel;
	}
