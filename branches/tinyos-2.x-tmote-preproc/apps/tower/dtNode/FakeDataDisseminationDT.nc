 ...
 * * DATE
 * *    $LastChangedDate: 2009-05-21 02:49:16 -0500 (Thu, 21 May 2009) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: dfacchin $
 * *
 * *	$Id: FakeDataDisseminationDT.nc 850 2009-05-21 07:49:16Z dfacchin $
 * *
...
/* Deformation/Temperature Node configuration */ 

// The sampling period (SP)
#define SAMPLING_PERIOD 1

// The reporting period (RP)
#define REPORT_PERIOD 1

// The numner of samples (N) 
#define SAMPLES 10

module FakeDataDisseminationDT {
  ...
  event void Boot.booted() {
    ...
    tuple<uint8_t, uint8_t, uint16_t, uint16_t, uint16_t> taskTuple =
      newTuple(
	       ...
	       actualField(DT_TASK),
               actualField(SAMPLING_PERIOD),
               actualField(REPORT_PERIOD),
               actualField(SAMPLES));
