 ...
 * *
 * *	$Id: TupleGateway.nc 300 2008-02-26 19:50:56Z mceriotti $
 * *
...
/*   bool startSync; */
  ...
  tuple<uint8_t, uint16_t, uint16_t, uint16_t> tree_building_tuple;
...
/*     call TimerWait.startOneShot(TUPLE_DISSEMINATION_WAIT_TIME); */
  ...
  task void handleSerialMsg(){
    tuple * t;
    tuple<uint8_t, uint16_t, uint16_t, uint16_t> *t_st;
    t = (tuple *) temp_serial_msg.data;   
    if (isOfType(t,tuple<uint8_t, uint16_t, uint16_t, uint16_t>)){
      t_st = (tuple<uint8_t, uint16_t, uint16_t, uint16_t> *) t;
      if (t_st->value0 == DATA_COLLECT_CTRL_TYPE){
        call TLObjects.copy_tuple((tuple *) &tree_building_tuple,t);
        call TS.out(&outId, FALSE, TL_LOCAL, RAM_TS, (tuple *)&tree_building_tuple);
      } else if (t_st->value0 == SYNCHRONIZATION_TYPE){
        call TimerWaitSynch.startOneShot(TUPLE_DISSEMINATION_WAIT_TIME);
        if (call TimerWaitDisseminate.isRunning())
          call TimerWaitDisseminate.startOneShot(2*TUPLE_DISSEMINATION_WAIT_TIME);
      }    
    } else {
  ...
  event void CollectionInfo.reliablePath(bool reliable){
    if (!reliable)
      call TimerWaitSynch.startOneShot(TUPLE_DISSEMINATION_WAIT_TIME);
  }
