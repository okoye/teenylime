 ...
 * *
 * *	$Id: SolarLight.java 989 2009-12-07 10:00:55Z mceriotti $
 * *
...
public class SolarLight implements _CollectionFeatureListener {
	...
	public SolarLight(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public SolarLight(String dir, boolean gui) {
		feature = new FeatureTower(FeatureTower.SOLAR_LIGHT, "SolarLight");
		if (dir.length() > 0) {
	...
	public void receivedSample(SourceId sourceId, Sample sample) {
		...
		if (console) {
			...
			try {
				...
				writer.write("\tSOLAR LIGHT: " + sampleValue.get(0));
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "SolarLight", "Solar Light",
					"Lux");
		return panel;
	}
	...
	public static Vector<Double> convert(int rawValue) {
		...
		double solarLight = (((double) rawValue / 4096 * 1.5) / 100) * 0.769 * 1e6;
		ret.add(new Double(solarLight));
