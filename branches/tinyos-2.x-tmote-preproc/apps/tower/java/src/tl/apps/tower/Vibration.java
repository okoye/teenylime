 ...
 * * DATE
 * *    $LastChangedDate: 2010-01-08 02:56:54 -0600 (Fri, 08 Jan 2010) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: mceriotti $
 * *
 * *	$Id: Vibration.java 1008 2010-01-08 08:56:54Z mceriotti $
 * *
			...
			for (int i = 0; i < dec.size(); i++) {
				...
				if (value.get(1) != -1) {
					String vibr_file = dirName + File.separator
							...
							+ "Vibration_" + cal.get(Calendar.YEAR) + "-"
							+ (1 + cal.get(Calendar.MONTH)) + "-"
							+ cal.get(Calendar.DAY_OF_MONTH) + "-"
							+ cal.get(Calendar.HOUR_OF_DAY) + "_"
							+ cal.get(Calendar.MINUTE) + "_"
							+ cal.get(Calendar.SECOND) + "_"
							+ sourceId.toString() + "_" + value.get(0) + ".txt";
					try {
					...
					String vibr_file = dirName + File.separator
							...
							+ cal.get(Calendar.MINUTE) + "_"
							+ cal.get(Calendar.SECOND) + "_"
							+ sourceId.toString() + "_" + value.get(0)
							+ "_incomplete.txt";
					try {
