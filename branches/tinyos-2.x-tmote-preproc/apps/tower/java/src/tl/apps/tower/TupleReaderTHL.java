 ...
 * *
 * *	$Id: TupleReaderTHL.java 990 2009-12-07 10:05:11Z mceriotti $
 * *
...
public class TupleReaderTHL implements _CollectionTupleReader {
	...
	public Hashtable<_CollectionFeature, Sample> read(Tuple tuple) {
		...
		if (data[0] == Constants.TEMP_HUM_LIGHT_END_SESSION) {
			ending = true;
		}
		int node_identifier = (data[1] << 8) + data[2];
		int period = (data[3] << 8) + data[4];
		int temperature = (data[5] << 8) + data[6];
		int humidity = (data[7] << 8) + data[8];
		int solarLight = (data[9] << 8) + data[10];
		int synthLight = (data[11] << 8) + data[12];
