 ...
 * *
 * *	$Id: Humidity.java 989 2009-12-07 10:00:55Z mceriotti $
 * *
...
public class Humidity implements _CollectionFeatureListener {
	...
	public Humidity(boolean gui) {
		this.gui = gui;
		this.console = false;
	}

	public Humidity(String dir, boolean gui) {
		feature = new FeatureTower(FeatureTower.HUMIDITY, "Humidity");
		if (dir.length() > 0) {
	...
	public void receivedSample(SourceId sourceId, Sample sample) {
		...
		if (console) {
			...
			try {
				...
				writer.write("\tHUMIDITY: " + sampleValue.get(0));
	...
	public static _ChartPanel getChartPanel(_CollectionGUIScenario scenario) {
		if (panel == null)
			panel = new TimeSeriesChart(scenario, "Humidity", "Humidity", "%");
		return panel;
	}

	public static Vector<Double> convert(int rawValue, double temperature) {
		Vector<Double> ret = new Vector<Double>();
		double humidity = (double) (-4.0 + 0.0405 * rawValue - 0.0000028 * (rawValue * rawValue));
		humidity = (double) ((temperature - 25.0) * (0.01 + 0.00008 * rawValue) + humidity);
		ret.add(new Double(humidity));
