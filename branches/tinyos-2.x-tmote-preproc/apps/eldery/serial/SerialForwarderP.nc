...
implementation {
  ...
  event void Boot.booted()
  {
    ...
    tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> fwPattern;
    
    fwPattern = newTuple(actualField(FORWARD_TUPLE), dontCare());
  ...
  task void forwardAttempt()
  {
    ...
    fwTuple = newTuple(actualField(FORWARD_TUPLE), dontCare());
  ...
  {
    tuple<uint8_t, uint16_t, uint8_t> *pt =
        (tuple<uint8_t, uint16_t, uint8_t> *) postureTuple;
    if (pt->value0 != POSTURE_TUPLE)
      return FAIL;

    notification->type = POSTURE_EVENT;
    notification->data.posture.node = pt->value1;
    notification->data.posture.posture = pt->value2;
    return SUCCESS;
  }
  ...
  void fowardTuple(tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> *fwTuple)
  {
    tuple *payload = (tuple *) fwTuple->payload_field;
    ...
    if (isOfType(payload, tuple<uint8_t, uint16_t, uint8_t>))
  ...
  {
    tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> *fwTuple =
        (tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> *) 
