...
implementation {
  ...
  void notifyFall()
  {
    tuple<uint8_t, uint16_t, uint8_t> fallTuple;
    // TODO check that this format remains like this
    tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> fwTuple;

    fallTuple = newTuple(actualField(POSTURE_TUPLE), actualField(TOS_NODE_ID), 
            actualField(FALL_OCCURED));
  ...
  void nofifyImmobility()
  {
    tuple<uint8_t, uint16_t, uint8_t> immobilityTuple;
    // TODO check that this format remains like this
    tuple<uint8_t, uint8_t[MAX_PAYLOAD_SIZE]> fwTuple;
    
    immobilityTuple = newTuple(actualField(POSTURE_TUPLE),
            actualField(TOS_NODE_ID), actualField(IMMOBILITY_OCCURED));
    
    fwTuple = newTuple(actualField(FORWARD_TUPLE), arrayField());
    call TLObjects.copy_tuple((tuple *) fwTuple.payload_field, 
            (tuple *) &immobilityTuple);
