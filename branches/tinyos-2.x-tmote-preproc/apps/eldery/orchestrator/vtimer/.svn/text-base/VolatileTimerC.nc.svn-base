...
generic configuration VolatileTimerC(int size) { 
  provides interface VolatileTimer;
}

implementation {
  components new TimerMilliC() as InternalTimer; 
  components new VolatileTimerM(size) as TimerM;
  components MainC;

  TimerM.Boot -> MainC;
  TimerM.InternalTimer -> InternalTimer; 

  VolatileTimer = TimerM.VolatileTimer;
}
