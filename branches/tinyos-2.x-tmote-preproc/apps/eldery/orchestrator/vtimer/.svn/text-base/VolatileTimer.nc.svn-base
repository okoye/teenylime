...
interface VolatileTimer {
  command int addTask(uint32_t wait_time);
  command error_t resetTask(int id, uint32_t wait_time);
  command error_t stopTask(int id);
  command uint32_t remaining(int id);
  event void fired(int id);
}
