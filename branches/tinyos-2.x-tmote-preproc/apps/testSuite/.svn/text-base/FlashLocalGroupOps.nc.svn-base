 ...
 * execution, the following should be printed: two reactions, two tuples
 * returned by rdg, two tuples returned by ing and no tuples returned by
 * ing.
 *
 * @author Stefan Guna
...
implementation {
  ...
  int number = 0;
  tuple<uint8_t, uint16_t> res1, res2;
  ...
  event void TimerApp.fired() {
    ...
    if (op % 5 == 0) {  
      ...
      call Leds.set(0);
      number = 0;
      call TS.rdg(&rdId, FALSE, TL_LOCAL, FLASH_TS, (tuple *) &p);
    } else if (op % 5 == 3) {
      number = 0;
      call TS.ing(&inAId, FALSE, TL_LOCAL, FLASH_TS, (tuple *) &p);
    } else {
      number = 0;
      call TS.ing(&inBId, FALSE, TL_LOCAL, FLASH_TS, (tuple *) &p);
    }
  }
  ...
  event void TS.tupleReady(TLOpId_t operationId, TupleIterator *iterator) { 
    ...
    if (tmp != NULL && number == 0) {
      ...
      number++;
      if (operationId.commandId != reactionId.commandId) {
        call TS.nextSplitTuple(operationId, iterator);
        return;
      }
    }
    if (tmp != NULL && number == 1 && operationId.commandId != reactionId.commandId) {
      call TLObjects.copy_tuple((tuple *) &res2, (tuple *) tmp);
      number++;
      call TS.nextSplitTuple(operationId, iterator);
      return;
    }
    if (tmp != NULL && operationId.commandId != reactionId.commandId) {
      number++;
      call TS.nextSplitTuple(operationId, iterator);
      return;
    }
