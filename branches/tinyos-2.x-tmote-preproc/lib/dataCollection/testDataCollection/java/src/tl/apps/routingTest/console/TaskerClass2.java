 ...
 * * DATE
 * *    $LastChangedDate: 2009-05-07 11:46:52 -0500 (Thu, 07 May 2009) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: mceriotti $
 * *
 * *	$Id: TaskerClass2.java 834 2009-05-07 16:46:52Z mceriotti $
 * *
...
public class TaskerClass2 implements _ConsoleDisseminator {
	...
	public Tuple getTuple() throws IllegalArgumentException {
		...
		System.out.println("CLASS 2 TASK DEFINITION");
		System.out.println("Insert the period between messages in seconds");
		try {
		...
		System.out
				.println("Insert the total number of messages (-1 for an infinite task)");
		try {
		...
		content.add(new Field().actualField(new Uint8(Constants.CLASS_2_TASK)));
		content.add(new Field().actualField(new Uint16(1)));
		content.add(new Field().actualField(new Uint16(period)));
		content.add(new Field().actualField(new Uint16(num_sessions)));
		content.add(new Field().actualField(new Uint16(0)));
