 ...
 * * DATE
 * *    $LastChangedDate: 2008-01-28 20:20:56 +0200 (Mon, 28 Jan 2008) $
 * * LAST_CHANGE_BY
...
implementation {
  ...
  command error_t SendTuple.send(TLTarget_t target, tuple **tuples, 
    ...
    pos = tupleMsg->data;
    pos += copyTuple((tuple *) pos, signal NeighborSystem.getNeighborTuple());

    for (i = 0; 
         i < tupleNumber && pos - (char *) tupleMsg < sizeof(TupleMsg);
         i++) {
      pos += copyTuple((tuple *) pos, tuples[i]);
    } 
  ...
  event void ReliableSend.reliableSendFailed(am_addr_t addr, message_t* msg){
    ...
    TLOpId_t opId;
    // TODO failedTuple should be added to the tuple space
    tuple *nghTuple, *first_tuple;

    nghTuple = (tuple *) failedTupleMsg->data;
    first_tuple = (tuple *) (failedTupleMsg->data + tuple_sizeof(nghTuple));
	...
	&& failedTupleMsg->operation != REACTION_FIRING) { 
      signal SendTuple.reliableOpFail(opId, (TLTarget_t)addr, first_tuple);
    }
  }
