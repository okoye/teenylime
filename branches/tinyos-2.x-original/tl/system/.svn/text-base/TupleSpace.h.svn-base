...
//#define newTuple(n, s...) newtTuple_(n)

// There is definitely something wrong with this function.
// In avrora, the stack size grows linearly with the number of newTuple calls
// a program makes.
...
#define fastActualField_uint16(a) (unsigned)(TYPE_ACTUAL | TYPE_UINT16_T), a

void fastNewTuple(tuple *t, uint8_t numFields, ...) {
  int i = 0;
  va_list ap;
  va_start(ap, numFields);

  while(i < numFields && i < MAX_FIELDS) {
    t->fields[i].type = (fieldType) va_arg(ap, unsigned);
    t->fields[i].value = va_arg(ap, fieldValue);
    if (getFieldType(&(t->fields[i])) == TYPE_RANGE_LOW_UP_IN || getFieldType(&(t->fields[i])) == TYPE_RANGE_LOW_UP_OUT) {
      numFields++;
    }
    i++;
