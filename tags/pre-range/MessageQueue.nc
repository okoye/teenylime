 ...
 * * DATE
 * *    $LastChangedDate: 2007-04-28 12:40:53 -0500 (Sat, 28 Apr 2007) $
 * * LAST_CHANGE_BY
 * *    $LastChangedBy: lmottola $
 * *
 * *	$Id: MessageQueue.nc 10 2007-04-28 17:40:53Z lmottola $
 * *
  ...
  */
  ...
  command result_t Send.send(uint16_t address, uint8_t length, TOS_MsgPtr msg) {
    ...
    if(addPendingMsg(address, length, msg) == SUCCESS) {
      // Initial Scattering
      call SendMsgTimer.start(TIMER_ONE_SHOT, 
			      ((TOS_LOCAL_ADDRESS + 1) * 128) % 4096);
      return SUCCESS;
    }
    else {
      return FAIL;
