 ...
 * *
 * *	$Id: ReliableGenericComm.nc 4 2007-04-27 15:22:42Z paolinux78 $
 * *
...
module ReliableGenericComm {
  provides {
    ...
    interface ReceiveMsg as ReliableReceive;
  }

  uses {
    interface SendMsg as Send;
    interface ReceiveMsg as Receive;
    interface Timer as OneShotTimer;
...
//     }
 ...
 /****************************************/  
  ...
  result_t sendReliableMsg(TOS_MsgPtr msg) {
    ...
    if(call Send.send(TOS_BCAST_ADDR, sizeof(ReliableMsg), msg) == SUCCESS) {
  ...
  command result_t StdControl.start() {
    active = TRUE;
    return call OneShotTimer.start(TIMER_ONE_SHOT, 
				   (TOS_LOCAL_ADDRESS * 1024) % UPDATE);
  }
  ...
  event result_t OneShotTimer.fired() {
    
    static bool initTimer = TRUE;
    
    if(initTimer) {
      call PeriodicTimer.start(TIMER_REPEAT, UPDATE);
      initTimer = FALSE;
    }
  ...
  event TOS_MsgPtr Receive.receive(TOS_MsgPtr msg){
    ...
    if(isReliableMsg(msg) 
	   ...
	   || rMsg->target == TOS_BCAST_ADDR)) {      
      beaconNeeded = TRUE;
      call OneShotTimer.start(TIMER_ONE_SHOT, BEACON_TIMEOUT);
    }
    ...
    if(messageReceived(msg) == SUCCESS) {
      ...
      signal ReliableReceive.receive(msg);

      while(!isNullMsg(&unorderedMsg)) {
	signal ReliableReceive.receive(&unorderedMsg);
  ...
  event result_t Send.sendDone(TOS_MsgPtr msg, result_t success) {
    return signal ReliableSend.sendDone(msg,success);
  }
